...
async def test_dark_motion_turns_on_bedroom_light(
    ...
    hass.states.async_set(bedroom_auto_on_entities["sleeping_mode"], "off")
    hass.states.async_set(bedroom_auto_on_entities["focus_mode"], "off")

    # These states satisfy the motion branch conditions.
    hass.states.async_set(bedroom_auto_on_entities["light"], "off")
    hass.states.async_set(bedroom_auto_on_entities["illuminance_sensor"], "5")
    hass.states.async_set(bedroom_auto_on_entities["motion_sensor"], "off")
    await hass.async_block_till_done()

    # The automation only reacts to the off -> on edge, so we create it here.
    hass.states.async_set(bedroom_auto_on_entities["motion_sensor"], "on")
    await hass.async_block_till_done()

    assert len(adaptive_lighting_calls) == 1
    assert adaptive_lighting_calls[0].data["lights"] == bedroom_auto_on_entities["light"]
...
async def test_sleeping_mode_blocks_bedroom_auto_on(
    ...
    hass.states.async_set(bedroom_auto_on_entities["sleeping_mode"], "on")
    hass.states.async_set(bedroom_auto_on_entities["focus_mode"], "off")
    hass.states.async_set(bedroom_auto_on_entities["light"], "off")
    hass.states.async_set(bedroom_auto_on_entities["illuminance_sensor"], "5")
    hass.states.async_set(bedroom_auto_on_entities["motion_sensor"], "off")
...
async def test_manual_light_on_in_dark_room_applies_adaptive_lighting(
    ...
    hass.states.async_set(bedroom_auto_on_entities["light"], "on")
    await hass.async_block_till_done()
